...
### Added — post-T2.3 hot-path bench gate (P series, 2026-05-06)
...
> crash on the patched code. The original document
> `docs/ena-mp-limitation.md` was renamed and rewritten to
> `docs/dpdk-mp-teardown-protocol.md` (commits `87e79d61`...`b9527d43`).
...
**Scope (2026-04-30 two-condition A/B):** ENA secondary can attach
(mempool / memzone OK) and raw burst calls against **idle** rings
work. The SIGSEGV requires **both** conditions simultaneously:
(1) primary doing high-rate `DpdkPoller`-driven I/O (Stream/Socket
send+poll loop), and (2) secondary driving `DpdkPoller::poll()` →
`DpdkUdpSocket` receive path. Neither condition alone is sufficient
— raw `rte_eth_rx_burst` on the secondary under heavy traffic does
not crash, and the full secondary machinery with an idle primary
runs cleanly (753 k samples, 50 k/s, no crash).

The in-repo sentinel (`repro_ena_mp_secondary_rxburst`) exercises
the idle-ring path only and is **expected to exit 9**. It is
preserved as a regression probe — if it ever exits 0 (idle-ring
SIGSEGV), that is new evidence. The full two-condition reproducer
lives on `diag/ena-mp-isolation-2` (intentionally not merged) with
scripts `/tmp/ena_mp_isolation.sh`, `/tmp/ena_mp_diag_minsec.sh`,
and `/tmp/ena_mp_diag_benign_primary.sh`.
