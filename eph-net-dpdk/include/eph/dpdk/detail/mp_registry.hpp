...
///       → upgrade secondaries.
///   v4: T2.3 trust-boundary HMAC tags (added 2026-05-05).
///   v5: T2.3 reverted (2026-05-06). Per the project's "tenants
///       trusted, defend against accidents" threat model, the wild-
///       pointer / ABA / use-after-free coverage is already provided
///       by the magic+version+claim_gen primitives below; HMAC tags
///       added implementation complexity without exercising in
///       production (no operator path actually flipped the flag).
///       Layout reverts to v3-equivalent: per-slot `hmac_tag[32]`
///       and `header.hmac_enabled` flag removed. Bumping to v5
///       (rather than back to v3) so any in-memory v4 hugepages
///       are hard-rejected at attach time — recovery: stop all
///       peers, daemon recreates the registry on next start.
inline constexpr uint32_t kMpRegistryVersion = 5;
