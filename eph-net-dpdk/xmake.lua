...
target("eph-net-dpdk")
    ...
    -- Include-order fix: vcpkg's DPDK port bundles openssl headers that
    -- conflict with aws-lc's (ASN1_NULL typedef, CRYPTO_THREADID, etc.).
    -- xmake's `add_packages` emits -isystem flags in an order we cannot
    -- control at the Lua level (package injection happens after target
    -- config). The gcc14-wrap/g++ wrapper reorders the command line so
    -- any -isystem path containing "aws-lc" appears before all others.
    -- See /tmp/gcc14-wrap/g++ (generated by dpdk-setup.sh or manually).
