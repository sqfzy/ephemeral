#!/usr/bin/env bash
# dpdk_mp_topology_e2e.sh — MpTopology variant of the MP e2e (reshape
# stage 4). Mirrors `dpdk_mp_e2e.sh` exactly except for the binary
# names: it drives `dpdk_mp_topology_primary` / `dpdk_mp_topology_secondary`
# which exercise the recommended path where the caller passes only
# `MpTopology::uniform(...)` and the library auto-derives queue +
# src_port segments via the shared hugepage registry.
#
