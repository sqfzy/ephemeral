#!/usr/bin/env bash
# dpdk_mp_e2e.sh — Orchestrate the DPDK primary+secondary end-to-end test.
#
# Picks up the two gtest binaries `dpdk_mp_primary` / `dpdk_mp_secondary`
# from the xmake build tree and runs them in coordinated roles:
...
PRIMARY_BIN="$BUILD_DIR/dpdk_mp_primary"
SECONDARY_BIN="$BUILD_DIR/dpdk_mp_secondary"

: "${EPH_MP_FILE_PREFIX:=eph_mp_e2e_$$}"
: "${EPH_MP_PORT_ID:=0}"
