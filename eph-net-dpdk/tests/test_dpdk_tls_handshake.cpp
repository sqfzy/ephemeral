...
/// Phase 7: verifies the DPDK TLS handshake path compiles end-to-end and
/// that the previously-blocked `create()` factory no longer bails out with
/// the Phase 5 BLOCKER sentinel.
///
...
/// assert here is the property that used to fail compilation / fail at
/// `create()` time before Phase 7 landed:
///
///   * DpdkTcpStream<C, /*EnableTls=*/true> compiles in a TU that also
///     includes aws-lc (the previous Phase 5 BLOCKER).
///   * The type still satisfies `eph::net::Stream` and `eph::net::Pollable`.
///   * `StreamConfig::tls` (phase-7-added) exists and is `eph::net::TlsConfig`.
...
// Concept checks — these used to be impossible under the Phase 5 BLOCKER
// because including `detail/tls_state.hpp` forced an aws-lc header into the
// TU that also included vcpkg-openssl via `eph::dpdk::tcp.hpp`.
static_assert(en::Stream<TlsRawStream>,
              "Phase 7: DpdkTcpStream<*, true> must satisfy Stream");
static_assert(en::Pollable<TlsRawStream>,
              "Phase 7: DpdkTcpStream<*, true> must satisfy Pollable");
...
TEST(DpdkTlsHandshake, CreateFailsInvalidConfigNotBlockerSentinel) {
    ...
    // Regression guard: the Phase 5 stub emitted a BLOCKER sentinel string.
    ...
    EXPECT_EQ(std::strstr(detail, "vcpkg-openssl"), nullptr)
        << "Phase 5 BLOCKER regression: detail mentions vcpkg-openssl — the "
