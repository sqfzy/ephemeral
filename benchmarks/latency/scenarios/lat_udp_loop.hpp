/// @file scenarios/lat_udp_loop.hpp
/// Reusable inner-loop function for the `lat_udp` scenario. Same shape
/// as `lat_tcp_loop.hpp` minus the TLS template parameter — UDP has no
/// TLS variant in this bench suite.
///
...
inline int run_lat_udp_loop(::bench::BenchCtx& ctx) noexcept {
    ...
    if (port == 0) {
        std::fprintf(stderr, "run_lat_udp_loop: scenarios.<...>.port required\n");
        return 1;
    }
    ...
    if (payload_size < ::bench::kTimestampBlockSize) {
        std::fprintf(stderr,
                     "run_lat_udp_loop: payload_size=%zu < kTimestampBlockSize=%zu\n",
    ...
    if (!ip_r) {
        std::fprintf(stderr, "run_lat_udp_loop: invalid mock_ip '%s': %s\n",
    ...
    if (ctx.view == nullptr) {
        std::fprintf(stderr, "run_lat_udp_loop: ctx.view is null in DPDK build\n");
        return 3;
    }
    auto& view = *ctx.view;
    ...
    sock_cfg.dpdk.wire = view.make_udp_config(local_src_port, port);
    ...
    if (!client_ip_r) {
        std::fprintf(stderr, "run_lat_udp_loop: invalid client_ip '%s': %s\n",
...
#endif
    if (!sock_r) {
        std::fprintf(stderr, "run_lat_udp_loop: Socket::create failed: %s\n",
                     sock_r.error().detail);
        return 3;
    }
    ...
    if (auto r = poller->add(sock.get()); !r) {
        std::fprintf(stderr, "run_lat_udp_loop: poller->add failed: %s\n",
                     r.error().detail);
        return 3;
    }
#endif
    ...
    while (::bench::monotonic_raw_ns() < t_deadline && !::bench::shutdown_requested()) {
        ...
        if (!send_r) {
            std::fprintf(stderr, "run_lat_udp_loop: send_to failed at sample %llu: %s\n",
        ...
        while (!got_echo && !::bench::shutdown_requested()) {
            ...
            if (::bench::monotonic_raw_ns() - t0 > kPerSampleTimeoutNs) {
                std::fprintf(stderr,
                             "run_lat_udp_loop: echo timeout at sample %llu\n",
                             static_cast<unsigned long long>(sample_idx));
                timed_out = true;
                break;
            }
