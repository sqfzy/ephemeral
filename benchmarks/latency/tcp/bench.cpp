...
int main(int argc, char** argv) {
    ...
    if (auto p = eph::utils::pin_thread_strict(cfg.client_cpu, "bench_tcp", policy); !p) {
        spdlog::error("pin_thread_strict: {}", p.error()); return 1;
    }

    // Rotating source port avoids TIME_WAIT on reconnect between payloads.
    static uint16_t src_port = 55600;
