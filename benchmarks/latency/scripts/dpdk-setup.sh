...
# │ benchmarks/latency/scripts/dpdk-setup.sh     host env prep (this file)  │
# │ benchmarks/latency/scripts/dpdk-teardown.sh  undo, restore kernel NIC   │
...
#   4. Save state to .dpdk_state at the repo root for dpdk-teardown.sh
#
# It does NOT:
...
# They were formerly at eph-dpdk/scripts/ — any stale doc references to
# that path are obsolete.
#
# 用法：sudo ./benchmarks/latency/scripts/dpdk-setup.sh [选项]
...
# and the build/ tree both sit at the repo root.
if PROJECT_DIR=$(git -C "$SCRIPT_DIR" rev-parse --show-toplevel 2>/dev/null); then
    :
else
    # Fallback: walk up until we find a CLAUDE.md + xmake.lua (repo root markers).
...
usage() {
    cat <<EOF
${BOLD}用法${RESET}：sudo ./benchmarks/latency/scripts/dpdk-setup.sh [选项]
...
${BOLD}示例${RESET}：
  sudo ./benchmarks/latency/scripts/dpdk-setup.sh                     # 自动检测
  sudo ./benchmarks/latency/scripts/dpdk-setup.sh --verbose
  sudo ./benchmarks/latency/scripts/dpdk-setup.sh --check-only
  sudo ./benchmarks/latency/scripts/dpdk-setup.sh -y
  sudo DPDK_PCI=0000:28:00.0 ./benchmarks/latency/scripts/dpdk-setup.sh

${BOLD}恢复${RESET}：
  sudo ./benchmarks/latency/scripts/dpdk-teardown.sh
EOF
}
...
detect_nic() {
    ...
    if [[ -n "$vfio_bound" ]]; then
        ...
        info "如果是上次未恢复，先运行：sudo ./benchmarks/latency/scripts/dpdk-teardown.sh"
    fi
...
confirm_action() {
    ...
    echo -e "     ${RED}⚠ 网卡 ${DPDK_IFACE} 将从内核消失${RESET}"
    echo ""
...
setup_hugepages() {
    ...
    if [[ "$actual" -lt "$NR_HUGEPAGES" ]]; then
        warn "请求 ${NR_HUGEPAGES} 页但只分配到 ${actual} 页"
        info "尝试：NR_HUGEPAGES=${actual} sudo $0"
    fi
...
report_results() {
    ...
    separator
    suggest "接下来："
    info "  sudo $PROJECT_DIR/benchmarks/latency/lat tcp --dpdk"
    suggest "恢复网卡："
    info "  sudo $PROJECT_DIR/benchmarks/latency/scripts/dpdk-teardown.sh"
}
