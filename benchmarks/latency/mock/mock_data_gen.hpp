...
///          "X":"NEW","p":"50000.00","q":"0.001","T":..., "T_recv":...}
///
...
/// @param tsc_send   TSC at response generation time ("T" field — for RX latency).
/// @param tsc_recv   TSC at order reception time ("T_recv" field — for TX latency).
///                   Pass 0 to omit (backward compatible).
/// @return Number of bytes written to buf, or 0 if the buffer is too small.
inline size_t generate_execution_report(char* buf, size_t buf_size,
    ...
    if (tsc_recv > 0) {
        n = std::snprintf(buf, buf_size,
            ...
            R"("X":"NEW","p":"50000.00","q":"0.001","T":%llu,"T_recv":%llu})",
            static_cast<int>(symbol.size()), symbol.data(),
            static_cast<int>(side.size()), side.data(),
            static_cast<unsigned long long>(tsc_send),
            static_cast<unsigned long long>(tsc_recv));
    } else {
        n = std::snprintf(buf, buf_size,
            R"({"e":"executionReport","s":"%.*s","S":"%.*s","o":"LIMIT",)"
