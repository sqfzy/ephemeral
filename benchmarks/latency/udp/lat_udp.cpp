...
/// been extracted to `benchmarks/latency/scenarios/lat_udp_loop.hpp`'s
/// `run_lat_udp_loop(BenchCtx&)`. main() here is a thin wrapper that
/// does Platform / Poller bring-up and delegates.
...
int main(int argc, char** argv) {
    ...
    const bench::Scenario* sc = bench_cfg.scenario("lat_udp");
    if (sc == nullptr) {
        std::fprintf(stderr, "lat_udp: [scenarios.lat_udp] not found in %s\n",
                     conf_path);
        return 1;
    }

    bench::pin_client_from_cfg(bench_cfg, "lat_udp");

    const std::size_t payload_size = sc->get_or<uint32_t>("payload_size", 256);
    const uint16_t port = sc->get_or<uint16_t>("port", 0);
    if (port == 0) {
        std::fprintf(stderr, "lat_udp: scenarios.lat_udp.port required\n");
        return 1;
    }
    ...
    return bench::scenarios::run_lat_udp_loop(ctx);
}
