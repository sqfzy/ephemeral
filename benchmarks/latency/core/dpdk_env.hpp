...
/// Shared DPDK bootstrap for all bench client binaries.
///
/// `DpdkBenchEnv::create(argc, argv, cfg)` performs the full init sequence:
///   1. Split argv at "--" (EAL args before, bench args after)
///   2. EalGuard::init()
///   3. Platform::create() (port, mempool, queues)
///   4. Parse local/server/gateway IPs
///   5. Get local MAC via rte_eth_macaddr_get()
///   6. ARP-resolve gateway MAC
///
/// The result is a move-only struct containing all the resources needed to
/// create UDP senders, TCP sessions, or WS transports over DPDK.
#pragma once
...
struct DpdkBenchEnv {
    ...
    /// Full factory with explicit server_ip.
