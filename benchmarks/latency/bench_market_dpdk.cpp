...
int main(int argc, char** argv) {
    ...
    for (int i = 0; i < app_argc; ++i) {
        ...
        if (arg == "--server-ip" && i + 1 < app_argc) cfg.server_ip = app_argv[++i];
            ...
            spdlog::info("Usage: bench_market_dpdk [EAL args] -- [app args]");
    ...
    // Start in-process mock server (kernel TCP on NIC-A)
    auto mock = bench::start_mock(cfg, /*order_mode=*/false);

    // DPDK connect with shared TransportConfig
    ...
    // Shared poll loop — same code as socket bench
    bench::run_market_poll_loop(*conn->transport, cfg, "Market Data Benchmark Results (DPDK)");
