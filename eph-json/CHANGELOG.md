...
### Added
- **Fused `BookTicker::parse(data, len)` fast path** (Binance). Single-pass specialisation that skips the generic `JsonView` intermediate and dispatches directly on 1-char keys into the target `BookTicker` fields. ~2-3× faster than `BookTicker::from(parse(data, len))` on representative bookTicker payloads; byte-for-byte identical field values, enforced by the `BinanceBookTickerParse.MatchesFromFlow` test. Unknown fields (multi-char keys, or future Binance additions) are silently skipped so existing callers stay forward-compatible with additive schema changes. Companion microbenchmark `BM_BinanceBookTickerParse` in `benchmarks/bench_json_parse.cpp` (direct competitor to `BM_JsonParseAndExtract`), plus 7 new `BinanceBookTickerParse.*` gtest cases covering `MatchesFromFlow`, `OptionalFieldsMissing`, `MissingSymbolReturnsNullopt`, `MissingBidReturnsNullopt`, `UnknownFutureFieldIsSkipped`, `MalformedJsonReturnsNullopt`, `FieldReorderingPreservesValues`.
