...
| 2026-05-01 13:19 | /report retro --auto | /pax --loop --auto eph-net-dpdk review：9 rounds / 8 productive commits（5 fix + 3 test + 1 docs）+ 1 no-op；silent-misuse + V3 整洁度 LENS → ESCALATION L1-L4 全 clean → UNTIL 触发；+448/-42 lines；同形闭合 join_dynamic 7 sites / Platform::attach 5 sites / MpRegistry+IcmpDirectory schema guards；⚠️ round 6 误改 CLAUDE.md 已还原；5 教训 4 action items | ddf34893 | retro-20260501-131928.md |
| 2026-05-02 06:35 | /report decision --auto | lat_ws DPDK p99 稳定性 ADR：6 轮 bisect (mlockall / FIFO+rt=-1 / IRQ steer / mock CFS / rt=999000) → 仅 mlockall 两端入选；TX max 7400k→3000k、TX stddev 4×、跨次 spread 16×→1.5×；FIFO/IRQ 全 reject 因 softirq starvation；NOHZ_FULL 路径预留 | a19b5e0a | decision-20260502-063512.md |
| 2026-05-04 02:00 | /report retro --auto | /pax --loop --auto review eph-net-dpdk + 实施：15 batches / 232 rounds / 160 commits / 9h09m wall；22 NaN-slip + 8 TLS/security + 7 race + 13 HFT pipeline + 13 新 bench + 2 e2e tests + 30 stage marker scrub；6 教训 8 action items；time-class UNTIL 硬墙正确执行（subagent diminishing-returns 转 ESCALATION 而非退出） | 29f7a687 | retro-20260504-020041.md |
| 2026-05-05 11:30 | /report primer --auto | eph-net-dpdk 框架快照（dev audience，post 2026-05-02 daemon-led reshape）：5 核心抽象契约 + RX 热路径数据流 + daemon-led MP setup + ICMP 跨 lcore 反馈 + build/test/run + 扩展点 + 14+ 条 gotcha + 演进时间轴；为后续 /pax --review --deep 做素材底稿 | fccd9202 | primer-20260505-113039.md |
| 2026-05-05 17:28 | bench rx hot path post-T2.3 | 23 cases vs 2026-04-23 baseline: 22 byte-identical, BM_ParseIcmp 7.29→7.51 ns (+3% within noise, well under 5% gate); confirms T2.3 HMAC trust-boundary work is hot-path-zero | 187bd4ab | bench-rx-hot-path-20260505.txt |
| 2026-05-06 10:19 | bench sysmetrics kernel-vs-DPDK TCP echo | 3 trial × 5min × 2 mode (lat_tcp); perf stat + /proc/<pid>/status + /proc/interrupts; cycles ~equal (832G vs 837G), but kernel +13% instructions / 5.9× branch-misses / 2.7× cache-misses / 13.6× ctxt-switches / +13M ens35 NIC IRQs (DPDK kills these). voluntary_ctxt=0 both — both run busy-poll under sustained load | (artifact-only) | bench-sysmetrics-tcp-echo-20260506/REPORT.md |
| 2026-05-06 13:05 | /report retro --auto | kernel vs DPDK TCP echo bench → cgroup shield 验证 → "shield 没用"复盘：原 -41% cache-miss 是 baseline trial-3 outlier 误差；replication 后真实改善 mean -15% / stddev -75%；6 个教训 + 系统性问题 + action items；用户一句"再跑一次"挽救错误结论 | (artifact-only) | retro-20260506-130522.md |
| 2026-06-01 10:42 | /report experiment --auto | 2026-04-22 全场景延迟 sweep（7 scenario × kernel/dpdk = 14 runs, echo 30s / push 120s, EC2 Graviton, commit 7869ea4）归档到 benchmarks/latency/docs：H1 DPDK p50 RTT 比 kernel 快 12-17%(echo) 成立 4/5 echo+1/2 push；H2 绑核收紧 DPDK RX p99(tcp -25%/ws -18%)；3 个 DPDK 异常标记(ex_order TX 253µs/UDP p99 262µs/ex_market 16ms MMPP 伪影) | c591c217 | benchmarks/latency/docs/experiment-lat-full-sweep-20260422.md |
