...
- **Framer** — `BasicFixFramer` satisfies the legacy `eph::net::MessageFramer` concept (defined in `eph-core`). Networking transports now consume `eph::core::StreamCodec`; the typical FIX integration is `KernelTcpStream<RawStreamCodec>` driving `FixSession::on_rx()` from the stream's `on_message` callback (see Quick Start). The framer remains available for callers that still need its `decode()` / `encode()` shape.
