...
| `test_kernel_drain`                   | `KernelTcpStream::drain()` happy path / state transitions / timeout. |
...
| `test_kernel_tls_state`               | Structural `KernelTcpStream<C, true>` TLS-path tests without a live server. |
