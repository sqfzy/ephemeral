...
///
/// Usage (NIC bound to DPDK via vfio-pci):
///   sudo ./ws_echo_client -a 0000:28:00.0 \
///       -- --host echo.websocket.org --local-ip 172.31.23.112 \
///          --gateway-ip 172.31.16.1 --msg "hello dpdk"
///
///   sudo ./ws_echo_client -a 0000:28:00.0 \
///       -- --host echo.websocket.org --port 443 --ws-path / \
///          --local-ip 172.31.23.112 --gateway-ip 172.31.16.1 \
///          --count 5 --interval 1000 --no-verify
///
...
int main(int argc, char** argv) {
    ...
    // ── 4–9 run in an inner scope so DPDK resources (Platform, Transport)
    ...
    // ── 7. Create DpdkTransport (TCP + TLS + WS) ─────────────────────────
    //
    ...
    // ── 8. Echo loop: send message, receive echo ─────────────────────────
    auto& tp = **transport;
