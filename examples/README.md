...
| `simple_hft.cpp`          | DPDK / template     | Canonical "simple HFT client": `Platform::launch` + ARP + DPDK-native DNS + `DpdkTcpStream<WsCodec, true>::create_and_attach` (TCP+TLS+WS+queue selection+srcport+Poller add in one call) + single-lcore burst loop + end-of-run metrics snapshot. | `eph-net-dpdk`, `eph-codec`                |
