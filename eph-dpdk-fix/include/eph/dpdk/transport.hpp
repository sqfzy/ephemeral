...
public:
    ...
    /// Send text as a WebSocket text frame (non-blocking).
...
private:
    ...
    void tx_loop() {
        ...
        ws::FrameTemplate ws_tmpl_bin  = ws::FrameTemplate::for_binary();
        ws::FrameTemplate ws_tmpl_text = ws::FrameTemplate::for_text();
        ...
        while (running_.load(std::memory_order_acquire)) {
            ...
            for (int i = 0; i < n; ++i) {
                auto& tmpl = (batch[i].opcode == ws::opcode::kText)
                           ? ws_tmpl_text : ws_tmpl_bin;
                size_t ws_len = tmpl.encode(
